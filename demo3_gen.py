...
def show_menu():
    ...
    print("5. Receive a compliment from the Pythoneers!")
...
def pythoneers():
    compliments = [
        "you create elegant, efficient, and effective solutions to complex problems.",
        "you have an excellent understanding of the underlying technology.",
        "you are able to create software that is both powerful and easy to use.",
        "you are able to collaborate effectively with other team members.",
        "you are a true professional in programming.",  
    ]
...
def Kerim():
    while True:
        ...
        print("\n"
            ...
            "with subfolders (mo - fr) for you to be prepared for your next week.\n"
            "Enter two dates from the start till the end of a week u wish.\n"
            "Within this folder it creates 5 Folder named: Mo - Fr\n"
            "with the Corrosponding dates u entered\n"
            "\n"
            "\n"
        ...
        # add the current year to the 'start_date_var' string
        start_date_var = start_date_var + "." + str(datetime.now().year)
        # add the current year to the 'end_date_var' string
        end_date_var = end_date_var + "." + str(datetime.now().year)
        ...
        # Converts the first user entered name as date (start_date) into datetime and formats(parse time) with 'strptime' into string ("%days.%month")
        end_date = datetime.strptime(end_date_var, "%d.%m.%Y")
        
        # Enter path where to create folder
        main_folder_path = input('Enter your path "/home/documents/". (enter "q" to quit): ')
        ...
        # Calculate the dates within the specified variables ('start_date' added(+) to object 'timedelta'(with nr of 'days=' set to 'd' it will generate a new sequence for each iteration and represent the duration or difference of days for each one.) to calculate the range between 'end_date' minus(-) 'start_date' and add '.days + 1')
